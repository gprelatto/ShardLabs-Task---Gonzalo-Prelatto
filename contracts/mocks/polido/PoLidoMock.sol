...
contract PoLidoMock is StMATIC {
    ...
    function withdraw(
        ...
        if (mockFailNextDepositOrRedeem) {
            ...
            revert("random mock failure from polido");
        }

        uint256 redeemable = StMATIC.getPooledEthByShares(_amount);

        // Simplification: only allow withdrawing buffered ether.
        require(redeemable <= bufferedEther, "Can only withdraw up to the buffered matic.");
    ...
    function _submit(
        ...
        uint256 sharesAmount = StMATIC.getSharesByPooledEth(deposit);
        if (sharesAmount == 0) {
    ...
    function _setSharesAndEthBalance(uint256 stMaticBalance, uint256 ethBalance) public {
        totalShares = stMaticBalance;
    ...
    function getSharesByPooledEth(uint256 _ethAmount) public view override returns (uint256) {
        ...
        return StMATIC.getSharesByPooledEth(_ethAmount);
    }
    ...
    function getPooledEthByShares(uint256 _sharesAmount) public view override returns (uint256) {
        ...
        return StMATIC.getPooledEthByShares(_sharesAmount);
    }
}
