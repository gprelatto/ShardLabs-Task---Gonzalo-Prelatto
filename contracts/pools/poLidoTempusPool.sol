...
import "../protocols/polido/IPoLido.sol";

contract PoLidoTempusPool is TempusPool {
    IPoLido internal immutable polido;
    bytes32 public constant override protocolName = "PoLido";
    ...
    function depositToUnderlying(uint256 amountBT) internal override returns (uint256 mintedYBT) {
        ...
        polido.submit{value: msg.value}(referrer);
    ...
    function withdrawFromUnderlyingProtocol(uint256, address) internal pure override returns (uint256) {
        require(false, "PoLidoTempusPool.withdrawFromUnderlyingProtocol not supported");
        return 0;
    }

    /// @return Updated current Interest Rate as an 1e18 decimal
    function updateInterestRate() internal view override returns (uint256) {
        return polido.getPooledEthByShares(1e18);
    }
